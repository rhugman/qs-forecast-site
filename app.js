/* qs-forecast dashboard. Plain JS, no build step. Reads data/index.json, data/<slug>.json, data/weather.json
   ...
   Chart colours follow qsfc/plot_style.py; the outlook scale (PCT_CLASSES) shares no swatch with them. The APA scale
   (APA_CLASSES) is the SNIRH traffic light: a level against the P20 and the median of its own calendar month over the
   whole record. Both classifications are offered; the map switches between them (MAP_SCALES), the chart, the chips and
   the status strip carry the APA one. */
'use strict';
...
// The APA / SNIRH reading of the same levels: a value against the P20 and the median of its own calendar month over the
// whole record (qsfc.pipeline.monthly_reference). Three classes, the traffic-light scale the SNIRH geovisualiser uses;
// the keys are qsfc.pipeline.APA_CLASSES. Both scales are offered on the map, `MAP_SCALES`.
const APA_CLASSES = [
  { key: 'below_p20', cls: 'a0', colour: '#c0392b', label: 'below P20', long: 'below the P20 of this month' },
  { key: 'p20_to_median', cls: 'a1', colour: '#e5a025', label: 'P20 to median', long: 'between the P20 and the median of this month' },
  { key: 'above_median', cls: 'a2', colour: '#2d8a3e', label: 'above median', long: 'above the median of this month' },
];
const APA_BY_KEY = new Map(APA_CLASSES.map((c) => [c.key, c]));
const MAP_SCALES = {
  outlook: { label: 'Outlook, 5 classes', legend: 'Outlook at the season end:' },
  apa: { label: 'APA, 3 classes', legend: 'Season-end median against the P20 and median of that month:' },
};
...
const LABELS = {
  ...
  gapModel: 'Model since the last measurement', p2080: 'Forecast P20 / P80', dry: 'Forecast P20, dry case',
  reference: 'Reference for the month: P20 and median',
};
...
function apaClass(key) { return APA_BY_KEY.get(key) || null; }
function apaColour(key) { const c = apaClass(key); return c ? c.colour : '#cccccc'; }
...
function segButtons(id, initial, onSelect, attr) {
  const seg = document.getElementById(id);
  if (!seg) return;
  const key = attr || 'range';
  const buttons = [...seg.querySelectorAll(`button[data-${key}]`)];
  const mark = (k) => buttons.forEach((b) => { b.classList.toggle('on', b.dataset[key] === k); b.setAttribute('aria-pressed', b.dataset[key] === k); });
  ...
  if (!seg.dataset.wired) {
    seg.dataset.wired = '1';
    buttons.forEach((b) => b.addEventListener('click', () => { mark(b.dataset[key]); seg._onSelect(b.dataset[key]); }));
  }
  mark(initial);
}
...
const overview = { sites: [], order: [], slug: null, range: '1y', scale: 'outlook', fan: null, map: null, table: null };
...
async function pageIndex() {
  ...
  segButtons('map-scale', overview.scale, (key) => overview.map.setScale(key), 'scale');
...
// The APA chip: one of the three classes, with the thresholds it was measured against in the tooltip.
function apaChip(key, p20, p50) {
  const c = apaClass(key);
  if (!c) return el('span', { class: 'chip none', text: 'no class', title: 'this month has too few measurements in the record for a P20' });
  const thr = p20 === null || p20 === undefined ? '' : ` (P20 ${fmt(p20, 1)} m, median ${fmt(p50, 1)} m)`;
  return el('span', { class: 'chip ' + c.cls, text: c.label, title: c.long + thr });
}

// Map: one circle marker per well filled by its class on the chosen scale, a dashed outline for the wells not yet
// verified, a ring on the selected well. Two scales (MAP_SCALES): `outlook`, the season-end median's percentile among
// the well's own past season ends in five classes, and `apa`, the same median against the P20 and the median of the
// season-end month in the three APA classes. The scale switch restyles the markers and rewrites the legend; nothing
// else on the page follows it. Returns {highlight(slug), setScale(key)}.
function renderMap(sites) {
  ...
  let scale = overview.scale || 'outlook';
  const fillOf = (s, key) => {
    if (key === 'apa') return apaColour(s.season_end_apa_class);
    return pctColour(s.season_end_percentile);
  };
  const labelOf = (s, key) => {
    if (key === 'apa') { const c = apaClass(s.season_end_apa_class); return c ? c.label : 'no class'; }
    const p = s.season_end_percentile;
    return p === null || p === undefined ? 'no outlook' : pctClass(Number(p)).label;
  };
  for (const s of sites) {
    ...
    const m = L.circleMarker([s.lat, s.lon], {
      ...
      fillColor: fillOf(s, scale), fillOpacity: 0.92,
    });
    m.bindTooltip(`${s.site_id} &middot; ${labelOf(s, scale)}${product ? '' : ' &middot; not yet verified'}`);
  ...
  const drawLegend = () => {
    if (!legend) return;
    legend.innerHTML = '';
    legend.appendChild(el('span', { text: MAP_SCALES[scale].legend }));
    for (const c of (scale === 'apa' ? APA_CLASSES : PCT_CLASSES)) {
      const sw = el('span', { class: 'sw' });
      sw.style.background = c.colour;
      legend.appendChild(el('span', { title: c.long || '' }, [sw, c.label]));
    }
    if (scale === 'apa') {
      const none = el('span', { class: 'sw' }); none.style.background = '#cccccc';
      legend.appendChild(el('span', { title: 'the season-end month has too few measurements in the record for a P20' }, [none, 'no class']));
    }
  ...
  drawLegend();
  const setScale = (key) => {
    if (!MAP_SCALES[key] || key === scale) return;
    scale = key;
    overview.scale = key;
    sites.forEach((s, i) => {
      markers[i].setStyle({ fillColor: fillOf(s, scale) });
      markers[i].setTooltipContent(`${s.site_id} &middot; ${labelOf(s, scale)}${s.role === 'product' ? '' : ' &middot; not yet verified'}`);
    });
    drawLegend();
  };
  return {
...
async function selectSite(slug) {
  ...
  renderStatusStrip(d, 'apa-strip');
  const nMembers = d.provenance && d.provenance.feeds ? Object.values(d.provenance.feeds)[0].n_members : null;
  setText('sel-note', `Measurements to the last one on ${d.anchor.last_dip}, ${d.anchor.age_days} days before issue, each filled by the class `
    + 'it falls in against the P20 and the median of its own calendar month; the dashed line '
...
function renderSelectedHead(s) {
  ...
  if (chip) {
    const wrap = el('span', { class: 'chips', id: 'sel-chip' },
      [outlookChip(s.season_end_percentile), apaChip(s.season_end_apa_class, s.season_end_reference_p20, s.season_end_reference_p50)]);
  ...
  if (figs) {
    ...
    const items = [
      ...
      s.season_end_reference_p20 === undefined || s.season_end_reference_p20 === null ? null
        : ['Against the month', apaClass(s.season_end_apa_class) ? apaClass(s.season_end_apa_class).label : 'no class',
           `P20 ${fmt(s.season_end_reference_p20, 1)} m, median ${fmt(s.season_end_reference_p50, 1)} m for that month`],
    ].filter(Boolean);
...
// The monthly reference (qsfc.pipeline.monthly_reference) as a step series over `dates`: each day takes the value of
// its own calendar month, null where that month has too few measurements for a threshold.
function referenceOnDates(ref, dates, key) {
  const by = new Map(ref.month.map((m, i) => [m, ref[key][i]]));
  return dates.map((d) => {
    const v = by.get(Number(d.slice(5, 7)));
...
function renderSiteTable(sites) {
  ...
  for (const s of outlookOrder(sites)) {
    ...
    tr.appendChild(el('td', {}, [apaChip(s.season_end_apa_class, s.season_end_reference_p20, s.season_end_reference_p50)]));
...
async function pageSite() {
  ...
  badges.appendChild(apaChip(d.season.apa_class, d.season.reference_p20, d.season.reference_p50));
...
function renderSiteCards(d, scen) {
  ...
  const cards = [
    ...
    se.reference_p20 === undefined || se.reference_p20 === null ? null
      : ['Against the month' + tag,
         apaClass(scen ? scen.season_end.apa_class : se.apa_class) ? apaClass(scen ? scen.season_end.apa_class : se.apa_class).label : 'no class',
         `P20 ${fmt(se.reference_p20, 1)} m and median ${fmt(se.reference_p50, 1)} m of the ${se.reference_n} measurements the record holds for that month`],
  ].filter(Boolean);
  node.innerHTML = '';
...
// the season end, nothing else (no model fit: the landing page shows what was measured and what is forecast).
// Both modes draw the APA reference of the month (`reference`): the P20 and the median of the well's own record for
// each calendar month as step lines, with the measurements filled by the class they fall in (APA_CLASSES). The
// reference is behind a toggle on the site page (`refId`) and on by default on the overview. `record` (qsfc.pipeline.record_block) gives the full record; without it the chart
...
function renderFanChart(d, opts) {
  const o = Object.assign({ target: 'fan', compact: false, rangeId: 'fan-range', climId: 'clim-toggle', refId: 'ref-toggle',
    residualId: 'residual', noteId: 'fan-note', initial: '1y' }, opts || {});
  const inert = { setBand: () => {} };
  ...
  // The APA reference of the month, appended after the band traces so setBand's indices (3..9) still hold.
  const refBlock = d.reference && Array.isArray(d.reference.month) ? d.reference : null;
  const refToggle = o.compact ? null : document.getElementById(o.refId);
  let refOn = refToggle ? refToggle.checked : Boolean(refBlock);
  const refTraces = [];
  // The reference steps are drawn over the current hydrological year and the horizon only: the thresholds are the same
  // in every year, so repeating them over the whole record would add a sawtooth and nothing else.
  const refStart = d.reference_status && d.reference_status.start ? d.reference_status.start : addDays(d.issue_date, -365);
  const refDates = dates.filter((x) => x >= refStart);
  if (refBlock && refDates.length) {
    const refName = o.compact ? LABELS.reference
      : `${LABELS.reference} (${refBlock.years[0]}-${refBlock.years[1]}, ${refBlock.n_dips} measurements)`;
    refTraces.push(traces.length);
    traces.push({ x: refDates, y: referenceOnDates(refBlock, refDates, 'p20'), mode: 'lines', line: { width: 1.2, color: apaColour('below_p20'), dash: 'dash', shape: 'hv' },
      name: refName, legendgroup: 'ref', visible: refOn, hovertemplate: 'P20 of the month %{y:.2f} m<extra></extra>' });
    refTraces.push(traces.length);
    traces.push({ x: refDates, y: referenceOnDates(refBlock, refDates, 'p50'), mode: 'lines', line: { width: 1.2, color: apaColour('above_median'), dash: 'dash', shape: 'hv' },
      showlegend: false, legendgroup: 'ref', visible: refOn, hovertemplate: 'median of the month %{y:.2f} m<extra></extra>' });
  }
  ...
  // Measurements: filled by the APA class of each one against its own month, outlined in the measurement black so they
  // still read as measurements. Without the classes in the JSON they stay plain black.
  const dipClasses = Array.isArray(dips.apa_class) ? dips.apa_class : null;
  const dipColours = dipClasses ? dipClasses.map(apaColour) : null;
  const dipTrace = traces.length;
  traces.push({ x: dips.date, y: dips.head_masl, mode: 'markers',
    marker: dipColours
      ? { size: rec ? 5 : 6.5, color: refOn ? dipColours : COLOURS.obs, line: { color: COLOURS.obs, width: 0.8 } }
      : { size: rec ? 4.5 : 6, color: COLOURS.obs },
    name: o.compact ? LABELS.dips : rec ? `${LABELS.dips} (${dips.n}, ${dips.date[0]} to ${dips.date[dips.date.length - 1]})` : `${LABELS.dips} (last 3 years)`,
    customdata: dipClasses ? dipClasses.map((k) => (apaClass(k) ? apaClass(k).label : 'no class')) : null,
    hovertemplate: dipClasses ? 'measured %{y:.2f} m (%{customdata})<extra></extra>' : 'measured %{y:.2f} m<extra></extra>' });
  ...
  const refBand = refBlock && refDates.length ? { p20: referenceOnDates(refBlock, refDates, 'p20'), p50: referenceOnDates(refBlock, refDates, 'p50') } : null;
  const visible = (x0) => yRange([
    ...
    refBand && refOn ? { x: refDates, y: refBand.p20 } : null, refBand && refOn ? { x: refDates, y: refBand.p50 } : null,
  ...
  if (refToggle && refTraces.length) {
    refToggle._onRef = () => {
      refOn = refToggle.checked;
      Plotly.restyle(node, { visible: refOn }, refTraces);
      if (dipColours) Plotly.restyle(node, { 'marker.color': [refOn ? dipColours : COLOURS.obs] }, [dipTrace]);
      apply(current);
    };
    if (!refToggle.dataset.wired) {
      refToggle.dataset.wired = '1';
      refToggle.addEventListener('change', () => refToggle._onRef());
    }
  }
...
// The status strip: one cell per month from the start of the hydrological year to the end of the horizon
// (qsfc.pipeline.reference_status), each coloured by its APA class, with a bar for the month's level and two ticks for
// that month's P20 and median. Measured months are solid; months carried by the model (before the issue date) or by the
// forecast (after it) are lighter with a dashed outline, because they are model output, not measurements. The bar
// heights share one scale, the range of everything drawn, so the strip reads as a series; the class, not the height,
// is the statement.
const MONTH_LETTERS = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
const STRIP_KINDS = { measured: 'measured', modelled: 'model on the observed weather', forecast: 'forecast', none: 'no value' };
function renderStatusStrip(d, targetId) {
  const node = document.getElementById(targetId || 'apa-strip');
  if (!node) return;
  const rs = d.reference_status;
  node.innerHTML = '';
  if (!rs || !Array.isArray(rs.months) || !rs.months.length) { node.hidden = true; return; }
  node.hidden = false;
  const values = [];
  for (const m of rs.months) for (const v of [m.value, m.p20, m.p50_reference]) if (v !== null && v !== undefined) values.push(Number(v));
  const raw = Math.max(...values) - Math.min(...values) || 1;
  const lo = Math.min(...values) - 0.12 * raw, hi = Math.max(...values) + 0.08 * raw;  // padded, so the lowest month is still a bar
  const pos = (v) => Math.max(0, Math.min(100, (100 * (Number(v) - lo)) / (hi - lo)));
  const list = el('ol', { class: 'strip-months' });
  for (const m of rs.months) {
    const c = apaClass(m.class);
    const [year, month] = m.month.split('-').map(Number);
    const cell = el('li', { class: `cell ${m.kind}${c ? ' ' + c.cls : ' none'}` });
    const track = el('span', { class: 'track' });
    if (m.p50_reference !== null && m.p50_reference !== undefined) {
      const t = el('span', { class: 'tick p50' }); t.style.bottom = `${pos(m.p50_reference)}%`; track.appendChild(t);
    }
    if (m.p20 !== null && m.p20 !== undefined) {
      const t = el('span', { class: 'tick p20' }); t.style.bottom = `${pos(m.p20)}%`; track.appendChild(t);
    }
    if (m.value !== null && m.value !== undefined) {
      const bar = el('span', { class: 'bar' });
      bar.style.height = `${pos(m.value)}%`;
      if (c) bar.style.background = c.colour;
      track.appendChild(bar);
    }
    cell.appendChild(track);
    cell.appendChild(el('span', { class: 'm', text: MONTH_LETTERS[month - 1].slice(0, 1) }));
    if (month === 1 || m === rs.months[0]) cell.appendChild(el('span', { class: 'y', text: String(year) }));
    const parts = [`${MONTH_LETTERS[month - 1]} ${year}`, STRIP_KINDS[m.kind] || m.kind];
    if (m.value !== null && m.value !== undefined) parts.push(`${fmt(m.value, 2)} m`);
    if (m.kind === 'forecast' && m.p05 !== undefined) parts.push(`90 % ${fmt(m.p05, 1)} to ${fmt(m.p95, 1)} m`);
    if (m.p20 !== null && m.p20 !== undefined) parts.push(`P20 ${fmt(m.p20, 2)} m, median ${fmt(m.p50_reference, 2)} m of ${m.n_reference} measurements`);
    parts.push(c ? c.long : 'no class for this month');
    cell.setAttribute('title', parts.join(' \u00b7 '));
    list.appendChild(cell);
  }
  node.appendChild(list);
  const key = el('p', { class: 'note strip-key' });
  key.appendChild(document.createTextNode(`Hydrological year ${rs.hydro_year} to the end of the horizon (${rs.horizon_end}). `));
  for (const c of APA_CLASSES) {
    const sw = el('span', { class: 'sw' }); sw.style.background = c.colour;
    key.appendChild(el('span', { class: 'key-item', title: c.long }, [sw, c.label]));
  }
  key.appendChild(el('span', { class: 'key-item' }, [el('span', { class: 'sw dashed' }), 'modelled, not measured']));
  key.appendChild(el('span', { class: 'key-line' },
    [document.createTextNode('The ticks inside each bar are that month\u2019s P20 and median over the whole record.')]));
